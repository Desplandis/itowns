...
function transformTexturedPlane(camera, distance, plane) {
    const Yreel = 2 * Math.tan(THREE.MathUtils.degToRad(camera.fov / 2)) * distance;
...
function initCamera(view, image, coord, EnhToOrientationUp, EnhToOrientationLookAt, rotMatrix,
    ...
    const camera = new THREE.PerspectiveCamera(fov, size[0] / size[1], distance / 2, distance * 2);
