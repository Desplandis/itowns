...
abstract class PointCloudNode extends THREE.EventDispatcher {
    ...
    get origin() {
        ...
        const centerCrsIn = proj.forward(this.center);
        this._origin = new Coordinates(this.crs)
            .setFromArray(proj.forward([centerCrsIn.x, centerCrsIn.y, 0]));
        return this._origin;
    }
    ...
    setOBBes(min: number[], max: number[]) {
        ...
        const zmin = this.source.zmin;
        const zmax = this.source.zmax;
        ...
        this.voxelOBB.box3D.setFromArray(cornersLocal);
        this.voxelOBB.position.set(...origin);
        this.voxelOBB.quaternion.copy(rotation).invert();

        this.voxelOBB.updateMatrix();
        this.voxelOBB.updateMatrixWorld(true);

        this.voxelOBB.matrixWorldInverse = this.voxelOBB.matrixWorld.clone().invert();

        this.clampOBB.copy(this.voxelOBB);

        const clampBBox = this.clampOBB.box3D;
        ...
        this.clampOBB.matrixWorldInverse = this.voxelOBB.matrixWorldInverse;
    }
    ...
    findCommonAncestor(node: this): this | undefined {
        if (node.depth == this.depth) {
            if (node.id == this.id) {
                ...
                return this.parent?.findCommonAncestor(node.parent as this);
            }
